...
# Vemos los temas, el máximo y el mínimo. Tenemos 46 categorias: del 0 a 45
np.min(y_train)
np.max(y_train)
...
def create_model(neuronasEntrada):
    ...
    modelo = Sequential([
            ...
            Dense(64, activation="relu"), # Podemos cambiarlo
            Dense(64, activation="relu"), # Ponemos cambiarlo
            Dense(46, activation='softmax') # Siempre debe así, sofmax para multiclase, 46 clases
    ])
...
###########################################
### Busqueda sistematica - Ejercicio 1 ###
###########################################
...
# Guardamos el mejor modelo encontrado
bestmodelRandom = random_search_result.best_estimator_.model_
bestmodelRandom.save('modeloPractica4RandomizedSearchCV_ej1.keras')
metricsRandomizedSearchCV = bestmodelRandom.evaluate(x_test, y_test) 
